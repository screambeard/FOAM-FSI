...
template<class Type>
scalar bdf2areaDdtScheme<Type>::deltaT_() const
{
...
template<class Type>
scalar bdf2areaDdtScheme<Type>::deltaT0_() const
{
...
scalar bdf2areaDdtScheme<Type>::deltaT0_(const GeoField& vf) const
{
...
template<class Type>
tmp<typename bdf2areaDdtScheme<Type>::fluxFieldType>
bdf2areaDdtScheme<Type>::fvcDdtPhiCorr
(
...
template<>
tmp<surfaceScalarField> bdf2areaDdtScheme<vector>::fvcDdtPhiCorr
(
    const volScalarField& rA,
...
)
{
    ...
    if (mesh().moving())
    {
        ...
        )
        {
            ...
            surfaceScalarField phi0 = phi.oldTime()*mesh().magSf()/(mag(Sf.oldTime())+dimensionedScalar("SMALL",dimArea,SMALL));
            surfaceScalarField phi00 = phi.oldTime().oldTime()*mesh().magSf()/(mag(Sf.oldTime().oldTime())+dimensionedScalar("SMALL",dimArea,SMALL));
            ...
            (
                ...
                (
                    ...
                    rDeltaT*ddtPhiCoeff
                   *(
                        coefft0*((UV0oV & mesh().Sf()) + fvc::interpolate(V0oV)*phi0)
                      - coefft00*((UV00oV & mesh().Sf()) + fvc::interpolate(V00oV)*phi00)
                    )/fvc::interpolate(1.0/rA)
                )
            );
...
template<class Type>
tmp<surfaceScalarField> bdf2areaDdtScheme<Type>::meshPhi
(
    const GeometricField<Type, fvPatchField, volMesh>& vf
)
{
