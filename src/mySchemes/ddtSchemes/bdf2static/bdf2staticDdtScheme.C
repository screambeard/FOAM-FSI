...
template<class Type>
scalar bdf2staticDdtScheme<Type>::deltaT_() const
{
...
template<class Type>
scalar bdf2staticDdtScheme<Type>::deltaT0_() const
{
...
scalar bdf2staticDdtScheme<Type>::deltaT0_(const GeoField& vf) const
{
    // Bug fix, Zeljko Tukovic: solver with outer iterations over a time-step
    // HJ, 12/Feb/2010
    // Bug fix, Thijs Gillebaart: in first time step -> solve backwardEuler insteadt of bdf2static. Especially for meshPhi.
...
bdf2staticDdtScheme<Type>::fvcDdt
(
...
template<class Type>
tmp<fvMatrix<Type> >
bdf2staticDdtScheme<Type>::fvmDdt
(
...
template<class Type>
tmp<typename bdf2staticDdtScheme<Type>::fluxFieldType>
bdf2staticDdtScheme<Type>::fvcDdtPhiCorr
(
...
template<>
tmp<surfaceScalarField> bdf2staticDdtScheme<vector>::fvcDdtPhiCorr
(
    const volScalarField& rA,
        ...
        if(U.dimensions() == dimVelocity && phi.dimensions() == dimVelocity*dimArea)
        {
            ...
            (
                ...
                (
                    ...
                    rDeltaT*ddtPhiCoeff
                   *(
                        coefft0*((UV0oV & mesh().Sf()) + fvc::interpolate(V0oV)*phi.oldTime())
                      - coefft00*((UV00oV & mesh().Sf()) + fvc::interpolate(V00oV)*phi.oldTime().oldTime())
                    )/fvc::interpolate(1.0/rA)
                )
            );
...
template<class Type>
tmp<surfaceScalarField> bdf2staticDdtScheme<Type>::meshPhi
(
    const GeometricField<Type, fvPatchField, volMesh>& vf
)
{
