...
\*---------------------------------------------------------------------------*/
#include "twoDoFRigidBody2Masses.H"
...
defineTypeNameAndDebug(twoDoFRigidBody2Masses, 0);
addToRunTimeSelectionTable(rigidBody, twoDoFRigidBody2Masses, dictionary);

// * * * * * * * * * * * * * * * * Private Member Functions* * * * * * * * * //
void twoDoFRigidBody2Masses::writeState(const scalar& time)
{
    //Body
...
void twoDoFRigidBody2Masses::writeHeader()
{
    if(writeToFile())
    {
...
scalar twoDoFRigidBody2Masses::getForce(const vector forces) const
{
    return forces.y();
}

scalar twoDoFRigidBody2Masses::getMoment(const vector moments) const
{
    return moments.z();
}


void twoDoFRigidBody2Masses::calculateMoments(const Field<vectorField>& patchForces){
...
scalar twoDoFRigidBody2Masses::calculateThetaDamping(const scalar& theta) const{
...
scalar twoDoFRigidBody2Masses::calculatePosition()
{
    ...
    if(integrationScheme()=="RK4")
    {
        ...
        for(int i=0;i<nStages;i++)
        {
            ...
            C[0][0] = m_;
            C[0][1] = mtheta_*Lcg2rc_*cos(theta+thetaGeom_);
            C[1][0] = mtheta_*Lcg2rc_*cos(theta+thetaGeom_);
            C[1][1] = Irc_;

            C.source()[0] = bodyForce - c_*dybody - k_*ybody + mtheta_*Lcg2rc_*pow(dtheta,2)*sin(theta+thetaGeom_);
    ...
    {
        FatalErrorIn("void twoDoFRigidBody2Masses::calculatePosition()")
...
void twoDoFRigidBody2Masses::readInitialStateIfPresent(const dictionary& dict)
{
...
twoDoFRigidBody2Masses::twoDoFRigidBody2Masses(const dictionary& dict,const fvMesh& mesh,const word name):
...
mtheta_(readScalar(dict.lookup("mtheta"))),
m_(0.0),
Irc_(readScalar(dict.lookup("Irc"))),
...
twoDoFRigidBody2Masses::~twoDoFRigidBody2Masses()
{
...
Foam::septernion twoDoFRigidBody2Masses::calculateTransformation(const Field<vectorField>& patchForces)
{
    //- Update forces
...
void twoDoFRigidBody2Masses::write()
{
    if(writeToFile())
    {
